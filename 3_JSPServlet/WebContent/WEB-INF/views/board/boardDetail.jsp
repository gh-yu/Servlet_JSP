			...
			<form action="<%= request.getContextPath() %>/boardUpdateForm.bo" id="detailForm" method="post">
				<table>
					...
					<tr>
						...
						<td>
							<%= b.getNickName() %>
							<input type="hidden" name="nickName" value="<%= b.getBoardWriter() %>">
						</td>
						<th>조회수</th>
						<td>
							<%= b.getBoardCount() %>
							<input type="hidden" name="count" value="<%= b.getBoardCount() %>">
						</td>
						<th>작성일</th>
						<td>
							<%= b.getCreateDate() %>
							<input type="hidden" name="date" value="<%= b.getCreateDate() %>">
						</td>
					</tr>
					<tr>
						<th>내용</th>
